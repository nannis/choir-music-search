...
/**
 * Script to add female choir music through the backend API
 * Uses the existing /add-songs endpoint instead of direct database access
 */
...
async function addFemaleChoirMusicViaAPI() {
  console.log('🎵 Adding female choir music via API...');
  
  try {
    // Use the Supabase Edge Function API endpoint
    const apiUrl = 'https://kqjccswtdxkffghuijhu.supabase.co/functions/v1/choir-music-api/add-songs';
    ...
    console.error('❌ Error adding female choir music:', error.message);
    process.exit(1);
  }
}
...
if (require.main === module) {
  addFemaleChoirMusicViaAPI();
}

module.exports = { addFemaleChoirMusicViaAPI, femaleChoirMusic };
