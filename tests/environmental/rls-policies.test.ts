...
const hasValidSupabaseConfig = () => {
  return SUPABASE_URL !== 'http://localhost:54321' && 
         SUPABASE_ANON_KEY !== 'test-anon-key' && 
         SUPABASE_SERVICE_ROLE_KEY !== 'test-service-role-key';
};

// Skip tests if Supabase is not properly configured
const skipIfNoSupabase = () => {
  if (!hasValidSupabaseConfig()) {
    console.log('⚠️  Skipping RLS tests: Supabase not configured for local testing');
    console.log('   Set VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY, and SUPABASE_SERVICE_ROLE_KEY to run these tests');
    return true;
  }
  return false;
};
