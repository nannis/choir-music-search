...
export interface SongRecord {
  ...
  sourceLink: string;
  audioLink?: string;
  source: 'IMSLP' | 'Hymnary' | 'ChoralNet' | 'MuseScore' | 'SundMusik' | 'ChorusOnline' | 'HalLeonard' | 'FluegelMusic' | 'CarusVerlag' | 'SchottMusic' | 'StrettaMusic' | 'CPDL' | 'Musopen' | 'Other';
  language?: string;
  voicing?: string;
  difficulty?: string;
  season?: string;
...
// ChorusOnline Parser for female choir arrangements with audio
class ChorusOnlineParser extends BaseParser {
  private proxyUrl = 'https://r.jina.ai/http://www.chorusonline.com/sheet-music-female-choir';
  
  constructor() {
    super(1500); // 1.5 second delay for ChorusOnline
  }
  ...
  async parse(_url: string): Promise<SongRecord[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`ChorusOnline proxy fetch failed: ${response.status}`);
      }
      
      const text = await response.text();
      const items: SongRecord[] = [];
      
      // Parse ChorusOnline patterns for female choir arrangements
      const titlePatterns = [
        /([^-\n]+)\s*-\s*([^-\n]+)\s*\(([^)]+)\)/g,  // "Title - Artist (Composer)"
        /([^,\n]+)\s*by\s*([^,\n]+)/g,  // "Title by Artist"
        /([^,\n]+)\s*arranged by\s*([^,\n]+)/g  // "Title arranged by Arranger"
      ];
      ...
      titlePatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(text)) !== null) {
          ...
          const composer = match[3] ? match[3].trim() : artist;
          
          // Filter out invalid entries
          if (title.length < 3 || artist.length < 2) continue;
          if (title.toLowerCase().includes('chorusonline') || title.toLowerCase().includes('sheet music')) continue;
          ...
          const songData: Partial<SongRecord> = {
            title,
            composer,
            description: `Modern arrangement from ChorusOnline for women's choir (${voicing}) with practice MP3 files`,
            sourceLink: 'https://www.chorusonline.com/sheet-music-female-choir',
            source: 'ChorusOnline',
            voicing,
            difficulty: 'Intermediate',
            theme: 'Popular',
      ...
      console.error('ChorusOnline parsing error:', error);
      return [];
    }
  }
  
  getUpdateFrequency(): string {
    return 'daily'; // ChorusOnline updates regularly
  }
}

// Hal Leonard Parser for professional women's choir collections
class HalLeonardParser extends BaseParser {
  private proxyUrl = 'https://r.jina.ai/http://www.halleonard.com/search?q=women%27s+choir';
  
  constructor() {
    super(2000); // 2 second delay for Hal Leonard
  }
  ...
  async parse(_url: string): Promise<SongRecord[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Hal Leonard proxy fetch failed: ${response.status}`);
      }
      
      const text = await response.text();
      const items: SongRecord[] = [];
      
      // Parse Hal Leonard patterns for women's choir collections
      const titlePatterns = [
        /([^-\n]+)\s*-\s*([^-\n]+)\s*\(([^)]+)\)/g,  // "Title - Composer (Arranger)"
        /([^,\n]+)\s*by\s*([^,\n]+)/g,  // "Title by Composer"
        /([^,\n]+)\s*arranged by\s*([^,\n]+)/g  // "Title arranged by Arranger"
      ];
      ...
      titlePatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(text)) !== null) {
          const title = match[1].trim();
          const composer = match[2].trim();
          const arranger = match[3] ? match[3].trim() : undefined;
          
          // Filter out invalid entries
          if (title.length < 3 || composer.length < 2) continue;
          if (title.toLowerCase().includes('halleonard') || title.toLowerCase().includes('search')) continue;
          
          // Determine voicing and difficulty based on title/content analysis
          let voicing = 'SSA'; // Default
          ...
          if (titleLower.includes('easy') || titleLower.includes('beginner')) {
            difficulty = 'Easy';
          } else if (titleLower.includes('advanced') || titleLower.includes('difficult')) {
            difficulty = 'Advanced';
          }
          ...
          const songData: Partial<SongRecord> = {
            ...
            composer,
            textWriter: arranger,
            description: `Professional arrangement from Hal Leonard for women's choir (${voicing}) with digital rehearsal tracks`,
            sourceLink: 'https://www.halleonard.com/search?q=women%27s+choir',
            source: 'HalLeonard',
            voicing,
            difficulty,
            theme: 'Sacred',
      ...
      console.error('Hal Leonard parsing error:', error);
      return [];
    }
  }
  
  getUpdateFrequency(): string {
    return 'weekly'; // Hal Leonard updates less frequently
  }
}

// CPDL Parser for free public domain women's choir music
class CPDLParser extends BaseParser {
  private proxyUrl = 'https://r.jina.ai/http://www.cpdl.org/wiki/index.php/Category:Women%27s_choir';
  
  constructor() {
    super(2000); // 2 second delay for CPDL
  }
  ...
  async parse(_url: string): Promise<SongRecord[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`CPDL proxy fetch failed: ${response.status}`);
      }
      
      const text = await response.text();
      const items: SongRecord[] = [];
      
      // Parse CPDL patterns for women's choir works
      const linkRegex = /\[([^\]]+)\]\((\/wiki\/[^)\s]+)[^)]*\)/g;
      let match: RegExpExecArray | null;
      ...
      while ((match = linkRegex.exec(text)) !== null) {
        ...
        // Determine voicing and period based on title/content analysis
        let voicing = 'SSAA'; // Default for CPDL
        let period = 'Renaissance';
        const titleLower = title.toLowerCase();
        
        if (titleLower.includes('ssa') || titleLower.includes('3-part') || titleLower.includes('three part')) {
          voicing = 'SSA';
        }
        
        if (titleLower.includes('baroque') || titleLower.includes('bach') || titleLower.includes('handel')) {
          period = 'Baroque';
        } else if (titleLower.includes('classical') || titleLower.includes('mozart') || titleLower.includes('haydn')) {
          period = 'Classical';
        } else if (titleLower.includes('romantic') || titleLower.includes('brahms') || titleLower.includes('schubert')) {
          period = 'Romantic';
        }
        ...
        const songData: Partial<SongRecord> = {
          title,
          composer,
          description: `Free public domain work from CPDL for women's choir (${voicing})`,
          sourceLink: `https://www.cpdl.org${path}`,
          source: 'CPDL',
          voicing,
          difficulty: 'Intermediate',
          theme: 'Sacred',
      ...
      console.error('CPDL parsing error:', error);
      return [];
    }
  }
  
  getUpdateFrequency(): string {
    return 'weekly'; // CPDL updates less frequently
  }
}
...
export class DataIngestionService {
  ...
  constructor(db: mysql.Pool) {
    ...
    this.parsers.set('ChorusOnline', new ChorusOnlineParser());
    this.parsers.set('HalLeonard', new HalLeonardParser());
    this.parsers.set('CPDL', new CPDLParser());
  }
